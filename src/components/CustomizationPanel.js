...
export default function CustomizationPanel({
  ...
  const [split, setSplit] = useState(initialSplit || null);
  ...
  const hasChanges =
    days !== initialDays ||
    split !== (initialSplit || null) ||
    JSON.stringify(exercises) !== JSON.stringify(initialExercises || []) ||
        ...
        <div className="border-t border-zinc-800/50 pt-8">
          ...
          <div className="mt-8">
            <button
              ...
              className="w-full rounded-xl bg-orange-500 px-6 py-3 text-sm font-semibold text-white hover:bg-orange-600 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
            >
              {busy ? (
